...
export default function FancyInput({placeHolder,value, setValue, type}: Props) {
  ...
  return (
    <div className={`flex h-fit w-full relative border-2 ${focus?"border-primary":"border-neutral"} rounded-lg px-2 items-center`}>
        ...
        <input autoFocus onChange={(e)=>{setValue(e.target.value)}} value={value} type={inputType} placeholder={!focus? placeHolder: ""} onFocus={()=>{setFocus(true)}} onBlur={()=>{setFocus(false)}} className={`flex items-center outline-0 rounded-lg h-8 w-full`} />
