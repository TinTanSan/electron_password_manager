...
export default function NewEntryForm({setShowForm}:props) {
    ...
    return (
        <div className='backdrop-blur-lg z-20 fixed w-screen h-screen top-0 left-0 flex flex-col justify-center items-center'>
            <form onSubmit={handleAdd} className='flex flex-col relative w-1/2 h-[80vh] border-2 border-base-300 bg-base-100 z-10 shadow-lg rounded-xl p-2 px-5 items-center gap-2'>
                {/* tab selector */}
                <div className='flex w-full h-10'>
                    <div className='flex gap-2 items-center'>
                        <button onClick={()=>{setTab(true)}} type='button' className={`h-8 transition-all duration-700 ${tab? "bg-neutral text-neutral-content border-none shadow-md/30 shadow-neutral": "bg-base-200 text-base-content border-[1px] border-base-content"}  rounded-lg w-20 items-center justify-center shrink-0 flex`}>Main</button>
                        <button onClick={()=>{setTab(false)}} type='button' className={`h-8 transition-all duration-700 ${!tab? "bg-neutral text-neutral-content border-none shadow-md/30 shadow-neutral": "bg-base-200 text-base-content border-[1px] border-base-content"} rounded-lg w-20 items-center justify-center shrink-0 flex`}>Extra</button>
                    </div>
                    {/* title */}
