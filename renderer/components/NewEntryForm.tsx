    ...
    return (
        <div className='backdrop-blur-lg z-20 fixed w-screen h-screen top-0 left-0 flex flex-col justify-center items-center'>
            <form onSubmit={handleAdd} className='flex flex-col relative w-1/2 h-[80vh] border-2 border-base-300 bg-base-200 z-10 shadow-lg rounded-xl p-2 items-center'>
                ...
                {tab ? 
                <div className='flex flex-col w-full h-full gap-5 p-2 bg-base-100 border-2 border-base-300 rounded-b-lg rounded-tr-lg text-normal'>
                   
                   <div className='text-subheading w-full h-14'>Set Main fields</div>
                   <div className='flex flex-col w-full rounded-lg pl-2 shrink-0 grow-0'>
                        <div className='flex text-nowrap w-18 shrink md:w-20 lg:w-24 items-center text-normal font-normal'>Title</div>
                        <input value={entry.title} placeholder='enter a title here' id='title' className='flex w-full px-2 outline-none z-0 rounded-md h-8  border-2 focus:shadow-lg shadow-base-300 focus:border-primary transition-all duration-300' onChange={handleChange} />
                    </div>
                   <div className='flex flex-col w-full rounded-lg pl-2 shrink-0 grow-0'>
                        <div className='flex text-nowrap w-18 shrink md:w-20 lg:w-24 items-center text-normal font-normal'>Username</div>
                        <input value={entry.username} placeholder='enter the username for this account' id='username' className='flex w-full px-2 outline-none z-0 rounded-md h-8  border-2 focus:shadow-lg shadow-base-300 focus:border-primary transition-all duration-300' onChange={handleChange} />
                    </div>
                    <div className='flex flex-col w-full shrink-0 grow-0 rounded-lg pl-2'>
                        <div className='flex w-18 text-normal items-center'>Password</div>
                        <div className=" flex w-full rounded-md h-10 items-center gap-1 px-1 border-2 focus-within:border-primary">
                    ...
                    <div className='flex flex-col pl-2 items-start w-full h-full grow shrink'>
                        <div className='flex text-nowrap w-full'>Notes:</div>
                        <textarea placeholder='Enter any notes here, for sensitive data (like security questions) head over to the extra tab and add an extra field with the sensitive marker checked.' value={entry.notes} id='notes' className='flex w-full h-full resize-none px-1 border-2 outline-none rounded-lg focus:border-primary transition-all duration-700 focus:shadow-lg/100 shadow-base-300' onChange={handleChange} />
                    </div>
                </div>
                :
